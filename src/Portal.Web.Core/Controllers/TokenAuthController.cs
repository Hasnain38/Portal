...
using Portal.Authentication.TwoFactor;
...
using Portal.Web.Authentication.JwtBearer;
using Portal.Web.Authentication.TwoFactor;
using Portal.Web.Models.TokenAuth;
    ...
    public class TokenAuthController : PortalControllerBase
    {
        ...
        private async Task<string> EncryptQueryParameters(long userId, Tenant tenant, string passwordResetCode)
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                .ToString(PortalConsts.DateTimeOffsetFormat));
